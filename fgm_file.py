...
from module.model_adv.fgm_torch import FGM
# from module.model_classification.lstm_model import LSTMModel
...
loss_file = "./result/train_history/second/loss_distribution_fgm.txt"
# 下面的是第一个数据集的参数
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        # 对抗训练
        # 0.01 --> 0.001
        fgm.attack(17.0)  # 在embedding上添加对抗扰动
        loss_adv_t = model(x)
...
def start_train(train_loader, test_loader, n_class, file_num):
    model = TextCnnModel(MAX_WORDS, EMB_SIZE, HID_SIZE, DROPOUT, n_class).to(DEVICE)
    print(model)
    # model = torch.nn.DataParallel(model1, device_ids=[0, 1])  # 多GPU使用
    optimizer = optim.Adam(model.parameters())
    best_acc = 0.0
    # PATH = "./result/model/model.pth"
    PATH = "./model_save/adversarial_attack/fgm/" + str(file_num) + "/" + model.__class__.__name__ + ".pth"
    print(PATH)
    best_pre = []
    loss_pre = []
    test_loss_s = []
    ...
    test_loss_file = "./result/train_history/loss_epoch/loss_test_fgm(text_cnn).txt"
    train_loss_file = "./result/train_history/loss_epoch/loss_train_fgm(text_cnn).txt"
    test_acc_file = "./result/train_history/loss_epoch/test_acc_fgm(text_cnn).txt"
    start = time.clock()
