...
def pad_fgm_tensor(x_train, y_train, x_test, y_test, n_class, file_name):
    print("attack-fgm")
    ...
    test_label = start_train(test_loader, file_name, n_class=n_class)
    ...
    print(f"fgm——malicious acc:{tn/(tn+fp):.4f}")
    # print(tp/(fn+tp))
    # with open(loss_file, "w") as f:
    #     f.writelines(list(map(lambda x: str(x) + '\n', loss_dis)))


def start_train(test_loader, file_name, n_class):
    model = TextCnnModel(MAX_WORDS, EMB_SIZE, HID_SIZE, DROPOUT, n_class=n_class).to(DEVICE)
    print(model)
    PATH = "./model_save/adversarial_attack/fgm/" + str(file_name) + "/" + model.__class__.__name__ + ".pth"
    print(PATH)
    best_pre = []
    loss_pre = []
