...
loss_file = "./result/train_history/second/loss_distribution_none.txt"
# 下面是第一个数据集的参数
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        optimizer.step()
        model.zero_grad()
        loss_store.append(round(loss.item(), 4))
...
def start_train(train_loader, test_loader, n_class, file_num):
    ...
    best_acc = 0.0
    PATH = "./model_save/" + str(file_num) + "/" + model.__class__.__name__ + ".pth"
    print(PATH)
    best_pre = []
    loss_pre = []
    test_loss_s = []
    ...
    test_loss_file = "./result/train_history/loss_epoch/loss_test_none(text_cnn).txt"
    train_loss_file = "./result/train_history/loss_epoch/loss_train_none(text_cnn).txt"
    test_acc_file = "./result/train_history/loss_epoch/test_acc_none(text_cnn).txt"
    start = time.clock()
